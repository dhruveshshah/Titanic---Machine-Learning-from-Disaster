...
submit.df <- data.frame(PassengerId = rep(892:1309), Survived = rf.6.preds)

write.csv(submit.df, file = "RF_6.csv", row.names = FALSE)
...
rf.5.1<-train(x=rf.train.5,y=rf.label,method="rf",tunelength=3, ntree=1000, trcontrol=ctrl.1)
...
table(rf.label[cv.10.folds[33]])
ctrl.3<-trainControl(method = "repeatedcv",number = 3, repeats = 10, index = cv.10.folds)
c1<-makeCluster(6,type="SOCK")
registerDoSNOW(c1)

rf.5.3<-train(x=rf.train.5,y=rf.label,method="rf",tunelength=3, ntree=1000, trcontrol=ctrl.2)
...
test.submit.df <- data.combined[892:1309, c("Pclass", "title", "family.size")]
# Make predictions
rf.5.preds <- predict(rf.5.3, test.submit.df)
table(rf.5.preds)
...
# Make predictions
rf.5.preds <- predict(rf.5.cv.3, test.submit.df)
table(rf.5.preds)
